...
## ⚡ Understanding the Dual-Port System

When you run the project, it launches **two separate servers** simultaneously for a seamless developer experience:

1.  **Frontend (UI)**: Runs on **Port 3000** (`http://localhost:3000`)
    - This is the interface you see in your browser.
    - Powered by Vite.
2.  **Backend (API)**: Runs on **Port 5000** (`http://localhost:5000`)
    - This handles database operations and AI logic.
    - The Frontend automatically sends requests to this port via a **Proxy**.

> [!IMPORTANT]
> Always access the application via **http://localhost:3000**. You don't need to visit port 5000 directly; it works in the background to power your UI!
