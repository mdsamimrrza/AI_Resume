...
export function ValidationTab({ resumeId }: { resumeId: number | null }) {
  const { data, isLoading } = useGetValidation(resumeId ?? 0, {
    query: {
      enabled: !!resumeId,
      queryKey: getGetValidationQueryKey(resumeId ?? 0),
    }
  });
