...
export function SuggestionsTab({ resumeId }: { resumeId: number | null }) {
  const { data, isLoading } = useGetSuggestions(resumeId ?? 0, {
    query: {
      enabled: !!resumeId,
      queryKey: getGetSuggestionsQueryKey(resumeId ?? 0),
    }
  });
